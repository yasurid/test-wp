...
class FlipkartApi extends RestClient {
    ...
    public function search($keyword, array $options)
    {
        ...
        //$response = str_replace('failed to connect: timeout', '', $response);
        //$response = str_replace('I should not be here', '', $response);
        //$response = str_replace('failed to authenticate: timeout', '', $response);
    ...
    {
        $this->setCustomHeaders(array('Fk-Affiliate-Id' => $this->getTrackingId(), 'Fk-Affiliate-Token' => $this->getToken()));
