...
class WebgainsModule extends AffiliateFeedParserModule {
    ...
    private function prepareResults($results)
    {
        ...
        foreach ($results as $key => $product)
        {
            ...
            if ((float) $content->price && isset($r['normal_price']))
                $content->priceOld = $r['normal_price'];

            if (isset($r['european_article_number']))
                $content->ean = $r['european_article_number'];
    ...
    public function doRequestItems(array $items)
    {
        ...
        foreach ($items as $key => $item)
        {
            ...
            if ((float) $items[$key]['price'] && isset($r['normal_price']))
                $content->priceOld = (float) $r['normal_price'];
            else
                $content->priceOld = 0;
        }
