...
class TradedoublerProductsModule extends AffiliateParserModule {
    ...
    public function doRequest($keyword, $query_params = array(), $is_autoupdate = false)
    {
        ...
        if (TextHelper::isEan($keyword))
            ...
            $results = $this->getApiClient()->search($keyword, $options); // keyword search
    ...
    private function prepareResults($results)
    {
        ...
        foreach ($results as $key => $r)
        {
            ...
            if (in_array($r['offers'][0]['availability'], array('N', 'Out Of Stock', 'No')))
    ...
    public function doRequestItems(array $items)
    {
        foreach ($items as $key => $item)
        {
            ...
            if (isset($result['offers'][0]['availability']) && in_array($result['offers'][0]['availability'], array('N', 'Out Of Stock', 'No')))
                $items[$key]['stock_status'] = ContentProduct::STOCK_STATUS_OUT_OF_STOCK;
            else
